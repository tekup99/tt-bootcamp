...
output_dir = "broadband_results"
os.makedirs(output_dir, exist_ok=True)

# Veri yükle
df = pd.read_parquet("../../../../tt_data/processed/Broadband.parquet")
...
for feat, val in sorted_importances:
    ...
    feature_percents.append((val / total_gain) * 100)

top_n = 9
feature_names_top = feature_names[:top_n]
feature_percents_top = feature_percents[:top_n]

plt.figure(figsize=(8, 6))
plt.barh(feature_names_top[::-1], feature_percents_top[::-1], color='navy')
plt.xlabel("Importance (%)")
plt.title("XGBoost Feature Importance (Top 9 by % Gain)")
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "broadband_feature_importance.png"), dpi=300)
plt.close()

# Confusion Matrix (Test) - tablo seklinde kaydetme
cm = confusion_matrix(y_test, test_pred)

# Bir tablo oluşturmak icin matplot kullanabiliriz
fig, ax = plt.subplots(figsize=(4, 2))
ax.set_axis_off()

# Tabloyu hazirlayacak veri
row_labels = ["Actual No Churn", "Actual Churn"]
col_labels = ["Pred No Churn", "Pred Churn"]
cell_text = [
...
table.scale(1, 2)  # tablo boyutunu ayarlayabilirsin (width, height)

plt.savefig(os.path.join(output_dir, "broadband_test_confusion_matrix_table.png"), dpi=300, bbox_inches='tight')
plt.close()

## --- Test verisinde threshold karsilastirma plotu --- 
test_proba = final_model.predict_proba(X_test)[:, 1]

plt.figure(figsize=(8, 5))

# 1) No Churn (0) icin distribution
sns.kdeplot(
    test_proba[y_test == 0], 
    fill=True, color='blue', alpha=0.3, 
    label='No Churn Probability Dist'
)

# 2) Churn (1) icin distribution
sns.kdeplot(
    test_proba[y_test == 1], 
    fill=True, color='red', alpha=0.3, 
    label='Churn Probability Dist'
)

# 3) Iki threshold'u dikey cizgiyle goster
...
# 5) Kaydet
comparison_path = os.path.join(output_dir, 'broadband_test_threshold_comparison.png')
...
test_metrics_path = os.path.join(output_dir, "broadband_test_metrics.txt")
